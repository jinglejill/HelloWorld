...
import { StyleSheet, Text, View, Dimensions, Image,TextInput,Alert,TouchableOpacity,TouchableHighlight,Picker,ScrollView } from 'react-native';
...
export class SpecialPriceSettingScreen extends Component
{
  ...
  loadMenu = () =>
  {
    ...
    fetch('https://jummum.co/app/dev_jor/JORSpecialPriceProgramGetList.php',
    {
      method: 'POST',
      headers: {
  ...
  stringToDate = (_date,_format,_delimiter,_time,_timeDelimiter) =>
  {
    ...
    saveDetails = () => {
      ...
      this.setState({showSpinner:true});
      fetch('https://jummum.co/app/dev_jor/JORSpecialPriceProgramUpdate.php',
      {
        method: 'POST',
        headers: {
                    'Accept': 'application/json',
    ...
    editSpecialPriceProgram = (item) =>
    {
      ...
      console.log("edit startDate:"+startDate);
      console.log("edit endDate:"+endDate);

      console.log("edit specialPriceProgram:"+item.SpecialPrice);
    ...
    {
      this.setState({showSpinner:true});
      fetch('https://jummum.co/app/dev_jor/JORSpecialPriceProgramDelete.php',
      {
        method: 'POST',
        headers: {
                    'Accept': 'application/json',
        ...
        specialPriceItem = this.state.specialPriceProgram.map((item,index)=>(
          <View key={index}>
            <View style={{display:'flex',flexDirection:'row',width:Dimensions.get('window').width,marginTop:10,alignItems:'center'}}>
              ...
              <Text style={[styles.label,{paddingLeft:5,textAlign:'center',width:95}]}>{item.StartDate}</Text>
              <View style={{borderRadius:5,backgroundColor:'#64DCC8'}}>
                <Text style={[styles.label,{paddingLeft:5,paddingRight:5,textAlign:'center',width:60,color:"#FFFFFF",fontFamily:"Prompt-SemiBold",lineHeight:28}]}>{item.StartTime}</Text>
              </View>
            ...
            <View style={{display:'flex',flexDirection:'row',width:Dimensions.get('window').width,marginBottom:10,alignItems:'center'}}>
              ...
              <Text style={[styles.label,{paddingTop:10,paddingLeft:5,textAlign:'center',width:95}]}>{item.EndDate}</Text>
              <View style={{marginTop:10,borderRadius:5,backgroundColor:'#64DCC8'}}>
                <Text style={[styles.label,{paddingLeft:5,paddingRight:5,textAlign:'center',width:60,color:"#FFFFFF",fontFamily:"Prompt-SemiBold",lineHeight:28}]}>{item.EndTime}</Text>
              </View>
    ...
    return (
      ...
      <View style={{flex:1}}>
        ...
        <Text style={styles.label}></Text>
        <Text style={styles.label}>วันที่ร่วมส่วนลด</Text>
